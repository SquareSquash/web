  ...
  RSpec.describe LdapAuthenticationHelpers, type: :model do
    describe "#log_in" do
      ...
      it "should return true if the LDAP entry exists and can bind" do
        ...
        expect(@controller.log_in(@user.username, 'password')).to eql(true)
      end
      ...
      it "should extract the username from an email address login" do
        ...
        expect(@controller).to receive(:log_in_user).once.with(@user)
        expect(@controller.log_in("#{@user.username}@#{Squash::Configuration.mailer.domain}", 'password')).to eql(true)
      end
      ...
      it "should return false if the LDAP entry does not exist" do
        ...
        expect(@controller.log_in(@user.username, 'password')).to eql(false)
      end
      ...
      it "should create a new user if one doesn't already exist" do
        ...
        expect(@controller.log_in('newuser', 'password')).to eql(true)
      end
    end
  end
end
