...
Dir[Rails.root.join('app', 'models', '**', '*.rb')].each do |f|
  # prevent loading a model twice; this can cause validations to run double
  require f unless ActiveSupport::Dependencies.loadable_constants_for_path(f).any?(&:safe_constantize)
end
